...
namespace flexMC::lineOptions {
    ...
    struct Option {
        ...
        Option(std::string token_value, const Token::Type &token_type, const bool &check_with_type,
               const std::vector<Option> &next_options) : value(std::move(token_value)),
                                                          type(token_type),
                                                          check_type(check_with_type),
                                                          options(next_options) {
        }

        Option(std::string token_value, const Token::Type &token_type,
               const bool &check_with_type) : value(std::move(token_value)),
                                              type(token_type),
                                              check_type(check_with_type) {
        }
    };
