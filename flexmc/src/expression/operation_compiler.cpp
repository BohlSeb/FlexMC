...
namespace flexMC
{
    Operation functionCompiler::compile(const size_t &num_args, Operands &stacks)
    {
        if (stacks.fSize() <= 0)
        {
    ...
    Operation
    functionCompiler::detail::compileScalar(const std::string &symbol, const size_t &num_args, Operands &stacks)
    {
    ...
    Operation
    functionCompiler::detail::compileReduce(const std::string &symbol, const size_t &num_args, Operands &stacks)
    {
        if (stacks.tSize() < num_args)
        {
            throw std::runtime_error("Function call compiler did not find enough argument operands in the stack");
    ...
    Operands::Type arrayCompiler::compile(const size_t &num_args, Operands &stacks)
    {
        if (num_args == 0)
        {
